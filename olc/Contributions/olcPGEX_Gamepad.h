...
std::vector<olc::GamePad> olc::GamePad::getGamepads() {
  ...
  auto enumFunction = [](LPCDIDEVICEINSTANCE lpddi, LPVOID pvRef) {
    if (!(lpddi->dwDevType & DI8DEVTYPE_GAMEPAD)) return DIENUM_CONTINUE;
  ...
  dev->EnumDevices(DI8DEVCLASS_GAMECTRL, enumFunction, (LPVOID)&result,
                   DIEDFL_ATTACHEDONLY);
...
inline olc::GamePad::GamePad(LPCDIDEVICEINSTANCE lpddi) {
  ...
  int min = 0x7fffffff;
  for (auto p : axisPositions) min = std::min(min, p);

  for (auto &p : axisPositions) p -= min;
...
olc::GamePad::GamePad(std::string path) {
  ...
  const int32_t axisCodes[]{ABS_Y, ABS_X,  ABS_RY,    ABS_RZ,
  ...
  if (ff) {
    ...
    if (ioctl(fd, EVIOCSFF, &effect) == -1) perror("Error:");
  }
...
void olc::GamePad::poll() {
  ...
  while (readEvent(event)) {
    if (event.type == EV_KEY && ((event.code & BTN_GAMEPAD) == BTN_GAMEPAD ||
      ...
      const int32_t buttonCodes[]{BTN_X,
                                  BTN_A,
                                  BTN_B,
                                  BTN_Y,
                                  BTN_TL,
      ...
      for (int i = 0; i < sizeof(buttonCodes) / sizeof(int32_t); i++) {
        ...
        if (event.code == buttonCodes[i]) {
          ...
          if (i >= 14) {
            ...
            if (i == 15) axes[6] = int(pressed);
            if (i == 16) axes[7] = -int(pressed);
            if (i == 17) axes[7] = int(pressed);
          }
    ...
    if (event.type == EV_ABS) {
      ...
      for (int i = 0; i < sizeof(axisCodes) / sizeof(int32_t); i++) {
        if (event.code == axisCodes[i]) {
          ...
          if (i == 4) handleButton(6, axes[4] >= 0.75f);
          if (i == 3) handleButton(7, axes[3] >= 0.75f);
        }
      }
    }
  }
...
std::vector<olc::GamePad> olc::GamePad::getGamepads() {
  ...
  int items = scandir(
      "/dev/input/", &list,
      [](const dirent *name) {
        return int(strncmp("event", name->d_name, 5) == 0);
      },
      alphasort);
  ...
  for (int i = 0; i < items; i++) {
    ...
    if (gp.valid) result.push_back(gp);
  }
...
olc::GamePad::GamePad() { valid = false; }

olc::GamePad olc::GamePad::selectWithButton(std::vector<olc::GamePad> &pads,
                                            olc::GPButtons b) {
  for (auto &gp : pads) {
    gp.poll();
    if (gp.getButton(b).bPressed) return gp;
  }
  GamePad nullPad;
  return nullPad;
}
...
float olc::GamePad::getAxis(olc::GPAxes a) {
  ...
  if (axis > 1) axis = 1;
  if (axis < -1) axis = -1;
  if (axis <= OLC_GAMEPAD_DEADZONE && axis >= -OLC_GAMEPAD_DEADZONE) axis = 0;

#ifdef WIN32
  if (!xInput && (a == GPAxes::TL || a == GPAxes::TR)) return (axis + 1) / 2;
#endif
  return axis;
}
...
int olc::GamePad::getAxisCount() { return axisCount; }

int olc::GamePad::getButtonCount() { return buttonCount; }
